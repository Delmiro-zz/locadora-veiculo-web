...
public class AluguelDao implements Serializable {
	...
	public void excluir(Aluguel aluguelSelecionado) {
		Aluguel aluguelBanco = manager.find(Aluguel.class, aluguelSelecionado.getCodigo());
		manager.remove(aluguelBanco);
		manager.flush();
	}

	@SuppressWarnings("unchecked")
	public List<Aluguel> buscarTodos() {
		return manager.createQuery("from tb_aluguel a").getResultList();
	}
	...
	public List<Aluguel> buscarPorDataDeEntregaEModeloCarro(Date dataEntrega, ModeloCarro modeloCarro) {
		...
		if (modeloCarro != null) {
			ParameterExpression<ModeloCarro> modelo = builder.parameter(ModeloCarro.class, "modelo");
			predicates.add(builder.equal(a.get("carro").get("modeloCarro"), modelo));
		}
